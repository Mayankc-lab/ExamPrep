...
import { BsCollectionPlayFill, BsFileEarmarkTextFill, BsTrash } from 'react-icons/bs';
...
function AdminDeshboard(){
    ...
    async function onCourseDelete(id){
        if(window.confirm("Are you Sure Want to delete the course ?")){
            ...
            if(adminPassword){
                const res= await dispatch(deleteCourse({ id, adminPassword }));
                if(res?.payload?.success){
                    await dispatch(getAllCourse())
                }
            } else {
                alert("Password required to delete course");
            }
        }
    }
    ...
    return (
        <HomeLayout>
                        ...
                        <tbody>
                            ...
                            return (
                                ...
                                <td className="flex items-center gap-4">
                                    ...
                                    <button
                                        className="bg-blue-500 hover:bg-blue-600 transition-all ease-in-out duration-300 text-xl py-2 px-3 rounded-md font-bold"
                                        onClick={() => navigate(`/course/addnote/${course?._id}`, { state: { ...course } })}
                                    >
                                    <BsFileEarmarkTextFill />
                                    </button>

                                    <button
                                        className="bg-purple-500 hover:bg-purple-600 transition-all ease-in-out duration-300 text-xl py-2 px-3 rounded-md font-bold"
                                        onClick={() => {
            ...
            <GoLiveModal 
                ...
                isLiveActive={selectedCourse?.liveSession?.isLive}
            />
        </HomeLayout>
    )
}
