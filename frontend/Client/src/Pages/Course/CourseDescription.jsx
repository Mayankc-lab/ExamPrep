...
function CourseDescripition() {
    ...
    const currentStatus = String(userData?.subscription?.status || '').toLowerCase();
    const isActiveSubscriber = currentStatus === 'active' || currentStatus === 'created';
                ...
                <div className=" flex flex-col items-center justify-center md:shadow-[0_0_10px_black]   md:w-[50rem]">
                    ...
                    <div className=" grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10 py-10 relative w-1/2 md:w-[80%] ">
                        ...
                        <div className=" space-y-3  text-xl">
                                ...
                                <button onClick={()=>navigate("/course/test", {state:{...state}})} className=" bg-slate-700 text-xl rounded-md font-bold px-5 py-3 w-full hover:bg-slate-600 transition-all ease-in-out duration-300">
                                    Test
                                </button>
                                {role === "ADMIN" || isActiveSubscriber ? (
