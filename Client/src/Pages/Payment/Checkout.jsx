...
function Checkout() {
    ...
    async function handleSubscription(e) {
        ...
        const hasActiveRazorpayKey = Boolean(razorpayKey && String(razorpayKey).trim() && !String(razorpayKey).includes('your_'));

        if (!hasActiveRazorpayKey || !subscription_id) {
            const subscribeResponse = await dispatch(purchaseCourseBundle());
            const resolvedSubscriptionId = subscribeResponse?.payload?.subscription_id || subscription_id;

            const verifyResponse = await dispatch(verifyUserPayment({
                razorpay_payment_id: 'local-payment',
                razorpay_subscription_id: resolvedSubscriptionId || 'local-subscription',
                razorpay_signature: 'local-signature'
            }));
